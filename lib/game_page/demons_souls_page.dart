...
class DemonsSoulsPage extends StatefulWidget {
  final bool isDarkTheme;

  const DemonsSoulsPage({super.key, required this.isDarkTheme});

  @override

  _DemonsSoulsPageState createState() => _DemonsSoulsPageState();
}

class _DemonsSoulsPageState extends State<DemonsSoulsPage> {
  List<int> _counters = [];
  ...
  void _loadCounters() async {
    ...
    setState(() {
      int numRuns = (prefs.getInt('DemonsSouls_numRuns') ?? 1);
      _runTitles = List.generate(numRuns, (index) => prefs.getString('DemonsSouls_RunTitle${index + 1}') ?? 'Run ${index + 1}');
      _counters = List.generate(numRuns, (index) => (prefs.getInt('DemonsSouls_Run${index + 1}') ?? 0));
    });
  }
  ...
  void _incrementCounter(int index) async {
    ...
    prefs.setInt('DemonsSouls_Run${index + 1}', _counters[index]);
  }
  ...
  void _removeRun(int index) async {
    ...
    prefs.setInt('DemonsSouls_numRuns', _runTitles.length);
    for (int i = 0; i < _runTitles.length; i++) {
      prefs.setInt('DemonsSouls_Run${i + 1}', _counters[i]);
      prefs.setString('DemonsSouls_RunTitle${i + 1}', _runTitles[i]);
    }
  }
  ...
  void _renameRun(int index, String newName) async {
    ...
    prefs.setString('DemonsSouls_RunTitle${index + 1}', newName);
  }
  ...
  void _addRun() async {
    ...
    prefs.setInt('DemonsSouls_numRuns', _runTitles.length);
    prefs.setInt('DemonsSouls_Run${_runTitles.length}', 0);
    prefs.setString('DemonsSouls_RunTitle${_runTitles.length}', 'Run ${_runTitles.length}');
  }
          ...
          children: <Widget>[
            ...
            Center(
              child: Padding(
                ...
                child: Image.asset(
                  'assets/images/Demons_Souls_logo.png',
