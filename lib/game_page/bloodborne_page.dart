...
class BloodbornePage extends StatefulWidget {
  final bool isDarkTheme;

  const BloodbornePage({super.key, required this.isDarkTheme});

  @override
  _BloodbornePageState createState() => _BloodbornePageState();
}

class _BloodbornePageState extends State<BloodbornePage> {
  List<int> _counters = [];
  ...
  void _loadCounters() async {
    ...
    setState(() {
      int numRuns = (prefs.getInt('Bloodborne_numRuns') ?? 1);
      _runTitles = List.generate(numRuns, (index) => prefs.getString('Bloodborne_RunTitle${index + 1}') ?? 'Run ${index + 1}');
      _counters = List.generate(numRuns, (index) => (prefs.getInt('Bloodborne_Run${index + 1}') ?? 0));
    });
  }
  ...
  void _incrementCounter(int index) async {
    ...
    prefs.setInt('Bloodborne_Run${index + 1}', _counters[index]);
  }
  ...
  void _removeRun(int index) async {
    ...
    prefs.setInt('Bloodborne_numRuns', _runTitles.length);
    for (int i = 0; i < _runTitles.length; i++) {
      prefs.setInt('Bloodborne_Run${i + 1}', _counters[i]);
      prefs.setString('Bloodborne_RunTitle${i + 1}', _runTitles[i]);
    }
  }
  ...
  void _renameRun(int index, String newName) async {
    ...
    prefs.setString('Bloodborne_RunTitle${index + 1}', newName);
  }
  ...
  void _addRun() async {
    ...
    prefs.setInt('Bloodborne_numRuns', _runTitles.length);
    prefs.setInt('Bloodborne_Run${_runTitles.length}', 0);
    prefs.setString('Bloodborne_RunTitle${_runTitles.length}', 'Run ${_runTitles.length}');
  }
          ...
          children: <Widget>[
            ...
            Center(
              child: Padding(
                ...
                child: Image.asset(
                  'assets/images/Bloodborne_logo.png',
