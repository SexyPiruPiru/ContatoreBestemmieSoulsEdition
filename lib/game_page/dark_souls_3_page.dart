...
class DarkSouls3Page extends StatefulWidget {
  final bool isDarkTheme;

  const DarkSouls3Page({super.key, required this.isDarkTheme});

  @override
  _DarkSouls3PageState createState() => _DarkSouls3PageState();
}

class _DarkSouls3PageState extends State<DarkSouls3Page> {
  List<int> _counters = [];
  ...
  void _loadCounters() async {
    ...
    setState(() {
      int numRuns = (prefs.getInt('DarkSouls3_numRuns') ?? 1);
      _runTitles = List.generate(numRuns, (index) => prefs.getString('DarkSouls3_RunTitle${index + 1}') ?? 'Run ${index + 1}');
      _counters = List.generate(numRuns, (index) => (prefs.getInt('DarkSouls3_Run${index + 1}') ?? 0));
    });
  }
  ...
  void _incrementCounter(int index) async {
    ...
    prefs.setInt('DarkSouls3_Run${index + 1}', _counters[index]);
  }
  ...
  void _removeRun(int index) async {
    ...
    prefs.setInt('DarkSouls3_numRuns', _runTitles.length);
    for (int i = 0; i < _runTitles.length; i++) {
      prefs.setInt('DarkSouls3_Run${i + 1}', _counters[i]);
      prefs.setString('DarkSouls3_RunTitle${i + 1}', _runTitles[i]);
    }
  }
  ...
  void _renameRun(int index, String newName) async {
    ...
    prefs.setString('DarkSouls3_RunTitle${index + 1}', newName);
  }
  ...
  void _addRun() async {
    ...
    prefs.setInt('DarkSouls3_numRuns', _runTitles.length);
    prefs.setInt('DarkSouls3_Run${_runTitles.length}', 0);
    prefs.setString('DarkSouls3_RunTitle${_runTitles.length}', 'Run ${_runTitles.length}');
  }
          ...
          children: <Widget>[
            ...
            Center(
              child: Padding(
                ...
                child: Image.asset(
                  'assets/images/Dark_Souls_3_logo.png',
