...
class SekiroPage extends StatefulWidget {
  final bool isDarkTheme;

  const SekiroPage({super.key, required this.isDarkTheme});

  @override
  _SekiroPageState createState() => _SekiroPageState();
}

class _SekiroPageState extends State<SekiroPage> {
  List<int> _counters = [];
  ...
  void _loadCounters() async {
    ...
    setState(() {
      int numRuns = (prefs.getInt('Sekiro_numRuns') ?? 1);
      _runTitles = List.generate(numRuns, (index) => prefs.getString('Sekiro_RunTitle${index + 1}') ?? 'Run ${index + 1}');
      _counters = List.generate(numRuns, (index) => (prefs.getInt('Sekiro_Run${index + 1}') ?? 0));
    });
  }
  ...
  void _incrementCounter(int index) async {
    ...
    prefs.setInt('Sekiro_Run${index + 1}', _counters[index]);
  }
  ...
  void _removeRun(int index) async {
    ...
    prefs.setInt('Sekiro_numRuns', _runTitles.length);
    for (int i = 0; i < _runTitles.length; i++) {
      prefs.setInt('Sekiro_Run${i + 1}', _counters[i]);
      prefs.setString('Sekiro_RunTitle${i + 1}', _runTitles[i]);
    }
  }
  ...
  void _renameRun(int index, String newName) async {
    ...
    prefs.setString('Sekiro_RunTitle${index + 1}', newName);
  }
  ...
  void _addRun() async {
    ...
    prefs.setInt('Sekiro_numRuns', _runTitles.length);
    prefs.setInt('Sekiro_Run${_runTitles.length}', 0);
    prefs.setString('Sekiro_RunTitle${_runTitles.length}', 'Run ${_runTitles.length}');
  }
